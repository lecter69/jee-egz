...
public class NauczycielPeselValidator implements Validator {
    public void validate(FacesContext ctx, UIComponent component, Object value) {
        ...
        List<Nauczyciel> list = em.createNamedQuery("Nauczyciel.findByPesel").setParameter("pesel", pesel).getResultList();
        em.close();
        if (list.size()>0)
